    ...
    <MainLayout>
      <div className="max-w-xl mx-auto mt-10 p-6">
        ...
        <p className="text-center mt-2 mb-5">
          Masukkan sebuah topik, dan biarkan AI membuat roadmap untuk Anda.
        </p>
        {/* ... */}
        <form action={formAction}>
          ...
          <button
            type="submit"
            className="w-full bg-color-brand text-text-negative mt-6 p-4 font-semibold rounded-xl hover:brightness-90 transition cursor-pointer disabled:opacity-50"
            disabled={isPending}
          >
        ...
        <>
          <form action={saveRoadmapAction}>
            ...
            <input
              type="hidden"
              name="roadmapData"
              value={JSON.stringify(generatedRoadmap)}
            />

            <button
              type="submit"
              className="bg-color-brand2 text-text-negative mt-6 px-4 py-2 font-semibold rounded-xl hover:brightness-90 transition cursor-pointer flex mx-auto disabled:opacity-50"
              disabled={isPending}
            >
              <span
