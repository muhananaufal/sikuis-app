...
const TreeNode = ({
  ...
  node: RoadmapItem & { x: number; y: number };
  onClick: (node: RoadmapItem) => void;
  isChild?: boolean;
  isMobile?: boolean;
}) => {
  return (
  ...
  if (isChildConnection) {
    ...
    return (
      ...
      >
        ...
        <polygon
          points={`${to.x - 6},${to.y - 3} ${to.x},${to.y} ${to.x - 6},${
            to.y + 3
          }`}
          fill="#2563eb"
        />
    ...
    return (
      ...
      >
        ...
        <polygon
          points={`${to.x - 5},${to.y - 6} ${to.x + 5},${to.y - 6} ${to.x},${
            to.y
          }`}
          fill="#374151"
        />
...
export default function TreeRoadmap({ RoadmapData }: TreeRoadmapProps) {
  ...
  const dataToUse =
    RoadmapData && RoadmapData.length > 0 ? RoadmapData : defaultMockData;
  ...
  const calculatePositions = (data: RoadmapItem[]) => {
    ...
    data.forEach((node, index) => {
      ...
      const childrenHeight =
        childrenCount > 0
          ? Math.max(childrenCount * childSpacing, minMainNodeSpacing)
          : minMainNodeSpacing;
      ...
      currentY += Math.max(
        childrenHeight + (isMobile ? 60 : 80),
        minMainNodeSpacing + (isMobile ? 60 : 80)
      );
    });
  ...
  const mainNodes = positionedNodes.filter((node) =>
    dataToUse.some((mainNode) => mainNode.id === node.id)
  );
  const childNodes = positionedNodes.filter(
    (node) => !dataToUse.some((mainNode) => mainNode.id === node.id)
  );
  ...
  const containerHeight = Math.max(
    ...
    dataToUse.reduce(
      (acc, node) =>
        acc +
        Math.max(
          node.children.length * (isMobile ? 45 : 60) + (isMobile ? 140 : 180),
          isMobile ? 200 : 260
        ),
      200
    )
  ...
  return (
    <div className="relative w-full">
      {/* Roadmap Container */}
      <div
        className={`relative mx-auto bg-gradient-to-b from-slate-100 to-slate-200 rounded-lg border border-slate-300 overflow-x-auto overflow-y-hidden ${
          isMobile ? "px-2" : ""
        }`}
        style={{
          ...
          {mainNodes.length > 1 &&
            mainNodes.slice(0, -1).map((node, index) => {
              ...
              return (
                <ConnectionLine
                  key={`main-${index}`}
                  from={{ x: node.x, y: node.y + (isMobile ? 40 : 50) }}
                  to={{ x: nextNode.x, y: nextNode.y - (isMobile ? 40 : 50) }}
      ...
      <Dialog open={!!selectedNode} onOpenChange={() => setSelectedNode(null)}>
        <DialogContent
          className={`${
            isMobile ? "max-w-[95vw] max-h-[85vh]" : "max-w-2xl max-h-[80vh]"
          } overflow-y-auto`}
        >
          <DialogHeader>
            <DialogTitle
